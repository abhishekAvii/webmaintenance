...
// Download PDF endpoint - bypasses client-side blob issues
app.post('/api/download-pdf', upload.none(), async (req, res) => {
  console.log('📥 Download PDF request received');
  try {
    const { pdfBase64, fileName } = req.body;
    console.log(`📄 Filename: ${fileName}, Base64 length: ${pdfBase64 ? pdfBase64.length : 'undefined'}`);

    if (!pdfBase64 || !fileName) {
      console.error('❌ Missing PDF data or filename for download');
      return res.status(400).json({
        success: false,
        message: 'PDFデータとファイル名が必要です'
      });
    }
    ...
    res.send(pdfBuffer);
    console.log(`✅ PDF "${fileName}" sent for download`);

  } catch (error) {
    console.error('❌ PDF download error:', error);
    res.status(500).json({
      success: false,
      message: `PDFダウンロード中にエラーが発生しました: ${error.message}`
    });
  }
});
